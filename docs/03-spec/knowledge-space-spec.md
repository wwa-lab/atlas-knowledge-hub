...
### Home And Dialogue Mode

- The home view contains a page title and subtitle for knowledge base management.
- A dialogue-first area appears above Knowledge Space cards.
- The dialogue area shows suggested questions, a selected knowledge context, an input area, quick-answer controls, attachment affordances, model selector, and send control.
- Suggested questions populate the dialogue input.
- Selecting the `IBM i Modernization` knowledge base can route to the space Ask experience.
- Knowledge Space cards remain visible below dialogue mode.

### Language Switching

- The UI provides a language control in the persistent shell.
...
- Switching language does not reset the current page, selected tab, selected knowledge context, or typed question.
...
- The UI provides a theme control in the persistent shell.
...
- The All Settings entry opens a modal/sheet over the current product view rather than replacing Home or Knowledge Space detail.
...
- The view shows configured model cards and model category filters, including all, chat, embedding, rerank, vision, and speech.
- The view includes an edit/configuration panel for a selected model.
- Supported provider/source examples include Ollama, OpenAI-compatible API, DeepSeek, GitHub Models, GitHub Copilot available models, Azure OpenAI, Anthropic, and other custom providers.
...
- Configuration fields include provider, source type, model name, display name, Base URL, API Key status, custom request headers, multimodal support, and thinking-parameter format.
- Prototype mode must never include real API keys. It may display `已配置` / `Configured` as mock state.
- Test connection and save actions are visual only in prototype mode and must not call external services.
...
- Prototype mode remains plain HTML, CSS, and JavaScript.
- No React, Vue, CDN, external images, or external network calls in prototype mode.
- Mock data only.
...
|---|---|
| REQ-KS-001 | Home cards render required fields. |
| REQ-KS-002 | Dialogue mode renders selected context and suggested questions populate input. |
...
| REQ-KS-013 | Language control switches primary UI copy between Chinese and English without changing view state. |
| REQ-KS-014 | Theme control switches day/night mode while preserving readability across all main views. |
| REQ-KS-015 | Settings opens Model Management with provider cards and an edit/configuration panel. |
