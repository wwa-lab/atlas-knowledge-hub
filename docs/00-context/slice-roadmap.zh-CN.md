...
| Wave 4 / Ask And Graph Productization | `ask-session-citations` | Session-scoped Trusted Ask answer citation snapshots，包含 safe source trace 与 review-aware evidence | ✅ 已按接受版 SDD 完成 implementation 与 verification | Answer governance、provider/model changes 与 production cost/quota controls 保持为 future work |
| Wave 4 / Ask And Graph Productization | `graph-from-wiki-extraction` | 从 eligible reviewed/published Wiki pages 与 source trace 中确定性抽取 graph nodes、edges 与 evidence snapshots | ✅ 已按接受版 SDD 完成 prototype implementation 与本地 verification | Model-assisted extraction、production graph layout/optimization、manual graph editing 与 external graph services 仍不在本切片范围内 |
| Wave 4 / Ask And Graph Productization | `answer-review-governance` | Ask answer 专用 governance states、reviewer-safe metadata、reusable eligibility、review-action API 与 Trusted Ask UI labels | ✅ 已按接受版 SDD 完成 prototype implementation 与本地 verification | Future answer reuse indexing、reviewer queues、production RBAC/audit workflow automation 与 production approval operations 仍不在本切片范围内 |
| Wave 4 / Ask And Graph Productization | `retrieval-quality-metrics` | 为 Trusted Ask runs 提供 deterministic quality metrics、evidence coverage、citation health、review eligibility、no-evidence refusal stats 与 safe diagnostics | ✅ 已按接受版 SDD 完成 implementation 与 verification | Production answer governance、online evaluation、provider/model changes 与 production cost/quota controls 保持为 future work |
| Wave 5 / Connector And Operations | `manual-url-knowledge-ingest` | Metadata-only manual URL source registration，包含 safe source trace、fetch intent、ingest status、eligibility metadata 与 review-required output | ✅ 已在预授权 SDD 下完成实现并通过 verification | Connector sync、scheduled/recursive crawl、真实 external fetch、production compliance scanning、connector secrets 与 approved Wiki/Ask/Graph 使用仍不在本切片范围内 |
| Wave 5 / Connector And Operations | `connector-sync-v0` | Adapter-first mock/local connector sync foundation，包含 connector registry、sync job/run/item status、source trace、safe errors 与 review-required output handoff | ✅ 已按接受版 SDD 完成本地实现与验证 | Real providers、OAuth/API keys、scheduled/background sync、webhooks、external network fetch、connector marketplace 与 direct approved Wiki/Ask/Graph use 仍不在本切片范围内 |
| Wave 5 / Connector And Operations | `worker-retry-dead-letter` | 面向 batch ingest、connector sync 与未来 async processing 的 deterministic local worker retry/dead-letter reliability contract | ✅ 已按接受版 SDD 完成本地实现与验证 | Production MQ、distributed worker cluster、scheduled production worker、exactly-once semantics、alert automation 与 real connector retry operations 仍不在本切片范围内 |
| Wave 5 / Connector And Operations | `deployment-monitoring-runbook` | Deployment readiness、post-delivery verification、monitoring signal catalog、alert severity、incident triage、rollback/recovery、safe evidence 和 closeout runbook | ✅ Documentation-only operations contract 已交付并本地验证 | Real monitoring platforms、alert channels、production deployment automation、production health/SLO dashboards、connector/worker automation 与 production incident tooling 保持为 future work |
| Wave 6 / Frontend Maintainability | `frontend-componentization` | 将单体 Vue `App.vue` 行为不变地结构化拆成特性组件/composables | ✅ SDD 已接受；T-001 至 T-007 已实现并本地验证 | Review/commit checkpoint；后续 router、URL 语义、后端/API 变更、新依赖或视觉重设计需另开 slice |
