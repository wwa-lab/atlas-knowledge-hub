...
| Wave 6 / Frontend Maintainability | `frontend-componentization` | 将单体 Vue `App.vue` 行为不变地结构化拆成特性组件/composables | 🔨 SDD 草案已生成；实现等待 SDD 接受 | 接受 SDD 后实现；不引入 `vue-router`、URL 语义、后端/API 变更、新依赖或视觉重设计 |
