...
|---|---|---|---|---|
| 1（前端） | `knowledge-space` | Vue 3 外壳复刻原型：首页、空间外壳、文档、Wiki、图谱、审核、Ask、设置 —— 仅 mock 数据 | 仅前端，无 API | ✅（T-KS-001→030） |
| 1（前端） | `folder-upload` | 文件夹/ZIP 上传 mock：文件清单、树、批次创建、mock 状态/报告 | 仅前端，无 API | 🔨 SDD 就绪（T-FU-001→013），待实现 |
...
| 3（适配器） | `model-adapter` | LLM/embedding 供应商置于 model 接口之后，secret 脱敏 | 适配器，禁止直连 | 🔒 需 Phase 2 |
