...
| Wave 4 / Ask And Graph Productization | `ask-session-citations` | Session-scoped Trusted Ask answer citation snapshots with safe source trace and review-aware evidence | ✅ Implemented and verified after accepted SDD | Answer governance, retrieval quality metrics, graph extraction, provider/model changes, and production cost/quota controls remain future work |
| Wave 4 / Ask And Graph Productization | `graph-from-wiki-extraction` | Deterministic graph nodes, edges, and evidence snapshots extracted from eligible reviewed/published Wiki pages with source trace | ✅ Prototype implemented and locally verified after accepted SDD | Retrieval-quality metrics, model-assisted extraction, production graph layout/optimization, manual graph editing, and external graph services remain out of scope |
| Wave 4 / Ask And Graph Productization | `answer-review-governance` | Ask-answer-specific governance states, reviewer-safe metadata, reusable eligibility, review-action API, and Trusted Ask UI labels | ✅ Prototype implemented and locally verified after accepted SDD | Future answer reuse indexing, reviewer queues, retrieval-quality metrics, production RBAC/audit workflow automation, and production approval operations remain out of scope |
