...
| Wave 6 / Frontend Maintainability | `frontend-componentization` | Behavior-preserving structural extraction of the monolithic Vue `App.vue` into feature components/composables | 🔨 SDD draft generated; implementation gated on SDD acceptance | Accept SDD, then implement without `vue-router`, URL semantics, backend/API changes, new dependencies, or visual redesign |
